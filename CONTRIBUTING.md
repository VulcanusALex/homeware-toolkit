...
./homeware --help                            # unified CLI
```

## Pull requests

1. Fork, branch, commit with clear messages (English).
2. Keep `python -m py_compile homeware_toolkit/*.py tools/*.py` and the test
...
reports are valuable. Run `./homeware support-bundle`, review `report.json`, and
attach the ZIP to a compatibility issue. Do not paste a session dump or HAR.

## Releasing

1. Bump `__version__` in `homeware_toolkit/__init__.py` and `version` in
...
3. Create a GitHub Release — the `release` workflow builds the wheel, source
   archive and standalone `homeware.pyz`, uploads them with `SHA256SUMS`, and
