"""Local web setup wizard for home-gateway-toolkit.

Running ``home-gateway setup --wizard`` starts a small HTTP server on ``127.0.0.1``
...
<title>home-gateway setup wizard</title>
<style>
...
<body>
<h1>home-gateway setup wizard <span class="muted">v{version}</span></h1>
...
<div id="step-setup" class="step">
  ...
  <p>This generates an RSA key locally and installs a LAN-only, key-only SSH
     ...
     <code>home-gateway ssh teardown</code>.</p>
...
<div id="step-done" class="step">
  ...
  <pre id="done-cmd"></pre>
  <p>Run <code>home-gateway doctor --key ~/.home-gateway-toolkit/id_rsa</code> any time
     to check health.</p>
</div>
...
class WizardState:
    ...
    def __init__(self, base_url: str, tls_fingerprint: str | None = None,
                 key_path: str = "~/.home-gateway-toolkit/id_rsa",
                 port: int = 2222) -> None:
...
def run_wizard(base_url: str, port: int = 2222,
               key: str = "~/.home-gateway-toolkit/id_rsa",
