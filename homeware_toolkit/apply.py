...
in cli.py) turns that into ``homeware diff`` (read-only preview) and
``homeware apply`` (idempotent convergence).
...
- :func:`load_config`  parse + strictly validate a JSON file into a
  ...
  :mod:`homeware_toolkit.firewall`, so anything accepted here is also safe for
  ``FW.ensure``.
    ...
    for rule in config.firewall_rules:
        ...
        if len(matches) > 1:
            ops.append(_op(
                ...
                "device; run 'homeware fw audit' and remove duplicates",
...
def render_plan(plan_dict: dict) -> str:
    """Human-readable, read-only rendering of a plan (``homeware diff``)."""
    lines = []
...
def run_diff(fw, config_path: str) -> tuple[dict, int]:
    """CLI entry for ``homeware diff``. Exit 0 in sync, 2 changes pending,
    1 assertions failed."""
...
def run_apply(fw, config_path: str, log=None) -> tuple[dict, int]:
    """CLI entry for ``homeware apply``. Exit 0 converged, 1 failure."""
    config = load_config(config_path)
    plan_dict = gather_plan(config, fw)
