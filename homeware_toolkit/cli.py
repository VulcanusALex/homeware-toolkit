"""Unified CLI for homeware-toolkit.

Usage: homeware [--base-url URL] [--json] [--quiet] [--version] <command> [options]
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="homeware", description=__doc__,
    ...
    p.add_argument("--tls-fingerprint",
                   help="pin the gateway certificate SHA-256 fingerprint "
                        "(see 'homeware session fingerprint')")
    ...
    p_setup.add_argument("--key", default="~/.homeware-toolkit/id_rsa",
    ...
    p_audit.add_argument("--state-file",
                         default="~/.homeware-toolkit/audit-state.json")
    ...
    for name in ("apply", "diff"):
        ...
        p_cfg.add_argument("-f", "--file", required=True,
                           help="path to the JSON config (see examples/homeware.json)")
    ...
    s_wg.add_argument("--out-dir", default="~/.homeware-toolkit/wireguard",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "session":
            ...
            if args.session_cmd == "fingerprint":
                ...
                if not args.json:
                    ...
                    print("pin it with: homeware --tls-fingerprint "
                          f"{fp} <command>")
        ...
        if args.command == "simulate":
            ...
            print(f"[simulate] try: homeware --base-url {gateway.base_url} "
                  "probe")
            try:
                while True:
                    time.sleep(3600)
