...
USER_AGENT = f"homeware-toolkit/{__version__} (own-network diagnostics)"
...
class GatewayClient:
    def __init__(self, base_url: str = DEFAULT_BASE_URL, timeout: float = 5.0,
        ...
        self.cookie_file = os.path.join(work_dir, "homeware_session_cookies.txt")
    ...
    def require_auth(self) -> None:
        if not self.is_authenticated():
            raise SessionExpired(
                "not authenticated; run 'homeware session login' or "
                "'homeware session import-cookie <har>' first")
