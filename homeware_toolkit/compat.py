...
def generate_compat_report(probe_result: dict,
    ...
    lines = [
        "# homeware-toolkit compatibility report",
        "",
        f"- Date (UTC): {today}",
    ...
    lines += [
        ...
        "",
        "- [ ] `homeware probe` output attached above (no credentials included)",
        "- [ ] Firmware version read from the web UI or `sysinfo` double-checked",
        "- [ ] `homeware doctor` run and result pasted below (optional)",
        "- [ ] `homeware verify` tried: yes / no / not tried",
