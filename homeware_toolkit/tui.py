"""Read-only curses dashboard ('homeware dashboard').
...
def wanwatch_state_provider(state_file: str) -> Callable[[], dict]:
    ...
    Reads the state file written by 'homeware wanwatch' so the dashboard can
...
def run_dashboard(providers: Providers, refresh: float = DEFAULT_REFRESH,
    ...
    if not sys.stdout.isatty():
        raise RuntimeError(
            ...
            "tty); use the --json output of 'homeware doctor' / 'homeware wanwatch' "
