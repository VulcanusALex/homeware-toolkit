...
def run_audit(base_url: str, port: int, key: str,
              state_file: str = "~/.nexxt-one-toolkit/audit-state.json",
    ...
    proc = ssh_run(
        ...
        "printf '%s\\n' __STATE__; test -s /etc/nexxt-toolkit/dropbear.nx.owned && echo managed")
    ...
    if ssh_ok:
        ...
        _check(checks, "firewall-audit", firewall_report["ok"],
               ...
               "run 'nexxt fw audit --json' and narrow broad WAN rules")
