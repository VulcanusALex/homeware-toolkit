...
DEFAULT_KEY = "~/.nexxt-one-toolkit/id_rsa"
    ...
    try:
        proc = subprocess.run(
            ["ssh-keygen", "-q", "-t", "rsa", "-b", "2048", "-N", "",
             "-C", "nexxt-one-toolkit", "-f", private_path],
...
def run_setup(base_url: str, port: int = 2222, key_path: str = DEFAULT_KEY,
    ...
    if compatibility != "strong-front-end-match" and not force:
        raise RuntimeError(
            "firmware fingerprint is not recognized; inspect 'nexxt probe' and "
    ...
    log("        - save a root-only rollback record under /etc/nexxt-toolkit")
