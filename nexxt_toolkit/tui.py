"""Read-only curses dashboard ('nexxt dashboard').
...
def wanwatch_state_provider(state_file: str) -> Callable[[], dict]:
    ...
    Reads the state file written by 'nexxt wanwatch' so the dashboard can
...
def run_dashboard(providers: Providers, refresh: float = DEFAULT_REFRESH) -> int:
    ...
    if not sys.stdout.isatty():
        raise RuntimeError(
            ...
            "tty); use the --json output of 'nexxt doctor' / 'nexxt wanwatch' "
