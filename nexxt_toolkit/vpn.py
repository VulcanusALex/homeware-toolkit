...
- An idempotent IPv6 firewall pinhole (UDP 51820 -> the WG server's LAN
  address) created through :class:`nexxt_toolkit.firewall.FW.ensure`.
...
def bootstrap_wireguard(fw, output_dir: str, *,
    ...
    ``fw`` may be None to only render configs; the manual ``nexxt fw ensure``
    ...
    if server_ipv6 is None:
        next_steps.append(
            ...
            "'nexxt wanwatch' for the current prefix).")
    if fw is None:
        next_steps.append(
            f"Open the pinhole manually once SSH is up: nexxt fw ensure "
            f"--name {rule_name} --proto udp --dest-ip <server-ipv6> "
            f"--dest-port {listen_port} --family ipv6 --key <key>")
    if endpoint:
        next_steps.append(
            f"Clients connect to {endpoint}; verify with 'nexxt inbound "
