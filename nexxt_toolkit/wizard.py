"""Local web setup wizard for nexxt-one-toolkit.

Running ``nexxt setup --wizard`` starts a small HTTP server on ``127.0.0.1``
...
<title>nexxt setup wizard</title>
<style>
...
<div id="step-setup" class="step">
  ...
  <p>This generates an RSA key locally and installs a LAN-only, key-only SSH
     ...
     <code>nexxt ssh teardown</code>.</p>
...
<div id="step-done" class="step">
  ...
  <pre id="done-cmd"></pre>
  <p>Run <code>nexxt doctor --key ~/.nexxt-one-toolkit/id_rsa</code> any time
     to check health.</p>
</div>
...
function finish() {
  show('step-done');
  $('done-cmd').textContent = 'ssh -i ~/.nexxt-one-toolkit/id_rsa -p 2222 \\\\n  -o HostKeyAlgorithms=+ssh-rsa -o PubkeyAcceptedKeyTypes=+ssh-rsa \\\\n  root@' + window.location.hostname.replace(/:$/, '');
}
</script>
</body>
</html>
...
class WizardState:
    ...
    def __init__(self, base_url: str, tls_fingerprint: str | None = None,
                 key_path: str = "~/.nexxt-one-toolkit/id_rsa",
                 port: int = 2222) -> None:
...
def run_wizard(base_url: str, port: int = 2222,
               key: str = "~/.nexxt-one-toolkit/id_rsa",
