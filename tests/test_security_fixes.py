...
from nexxt_toolkit import client as client_mod
from nexxt_toolkit import ssh as ssh_mod
from nexxt_toolkit import transfer
...
class TargetValidationTest(unittest.TestCase):
    def test_valid_targets(self):
        for target in ("/tmp/x", "/etc/nexxt-toolkit/authorized_key",
...
class KnownHostsPathTest(unittest.TestCase):
    def test_creates_dir_0700_and_file_0600(self):
        with tempfile.TemporaryDirectory() as tmp:
            kh_dir = os.path.join(tmp, ".nexxt-one-toolkit")
...
class TofuSshRunTest(unittest.TestCase):
    def _run(self, **kwargs):
        with mock.patch.object(
                ssh_mod, "known_hosts_path",
                return_value="/home/u/.nexxt-one-toolkit/known_hosts"
                ) as mock_kh, \
    ...
    def test_default_is_tofu_accept_new(self):
        ...
        self.assertIn(
            "UserKnownHostsFile=/home/u/.nexxt-one-toolkit/known_hosts", cmd)
